...
class _MockFixture(MockDatabaseTest):
    ...
    def _default_lookups(self):
        ...
        self.db.query(Client).filter_by(identifier="12345").\
                one.side_effect = NoResultFound

        # no result for 12346
        self.db.query(Client).filter_by(identifier="12346").\
                one.side_effect = NoResultFound
    ...
    def _auth_fixture(self, created_at=None, client=None, account=None):
        ...
        validate_session_q = self.db.query(AuthSession).\
    ...
    def _client_fixture(self):
        ...
        q = self.db.query(Client).filter_by(identifier="12345")
...
class CreateSessionTest(_MockFixture):
    ...
    def test_login(self):
        ...
        account = self.db.add.mock_calls[1][1][0]
        auth_session = self.db.add.mock_calls[2][1][0]
        ...
        response = start_session(request)
        auth_session_2 = self.db.add.mock_calls[3][1][0]
